...
import { LedgerX402PaymentHeaders, PayingHttpClient, parseHttpJson } from "../src/index.ts";
...
describe("paying HTTP client", () => {
  test("enforces Permit2 network policy before signing", async () => {
    let signatures = 0;
    ...
    await expectRejection(headers.create(required("eip155:31337", "0x3333333333333333333333333333333333333333", "1"), "eip155:31337"), "no allowed aqua");
  ...
  test("settles an already received Aqua 402 without probing the endpoint again", async () => {
    ...
    const client = new PayingHttpClient({
      ...
      paymentHeaders: {
        create: (_paymentRequired, network) => {
    ...
    const response = await client.retryPayment(requiredResponse, new URL("https://aqua.example/v1/trades"), { method: "POST" }, "eip155:31337");
    expect((await parseHttpJson(response)).body).toEqual({ ok: true });
    expect(requests).toBe(1);
    expect(selectedNetwork).toBe("eip155:31337");
  });
});
