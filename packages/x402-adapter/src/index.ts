...
export interface FundingOperation {readonly operationId:string;readonly sellToken:RuntimeManifest["fixtures"]["tokens"][number]["address"];readonly vault:RuntimeManifest["fixtures"]["tokens"][number]["address"];readonly atomicAmount:string}
...
export const exactPermit2UpfrontRequirement=(manifest:RuntimeManifest,input:FundingOperation):PaymentRequired=>{
  ...
  if(manifest.chain.id!==31_337||manifest.contracts.permit2.address!==PERMIT2_ADDRESS||manifest.contracts.x402ExactPermit2Proxy.address!==X402_EXACT_PERMIT2_PROXY)throw new Error("Manifest does not contain the verified canonical Permit2/x402 deployment");
  if(!manifest.fixtures.tokens.some((token)=>token.address===operation.sellToken))throw new Error("Sell token is not a verified manifest asset");
  const accepted:PaymentRequirements={scheme:"exact",network:"eip155:31337",asset:operation.sellToken,amount:operation.atomicAmount,payTo:operation.vault,maxTimeoutSeconds:300,extra:{assetTransferMethod:"permit2",paymentFlow:"upfront",paymentIdentifier:paymentIdentifier(operation.operationId),permit2:PERMIT2_ADDRESS,proxy:X402_EXACT_PERMIT2_PROXY}};
  return {x402Version:2,error:"Payment is required before order activation",resource:{url:manifest.services.mcpUrl,description:"Fund an immutable Ledger-owned Aqua order vault"},accepts:[accepted]};
};
