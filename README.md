...
2. **Act 2 — every new MCP Jam stdio connection.** Add one stdio server: `bun apps/mcp-bridge/src/main.ts --prod` with `AQUA_API_URL=https://vercel-henna-gamma-46.vercel.app`, `AQUA_RPC_URL`, and `XDG_STATE_HOME`. MCP `initialize` opens the Room in the default browser. Type the enrolled owner, Knock, confirm on the Security Key app. Loopback `http://127.0.0.1:$AQUA_OAUTH_CALLBACK_PORT/callback`. Trades in that session only attach or refresh the bearer; do not Knock to send a trade. Device stays dark on market trades unless a 409 needs the Ethereum app. Set `AQUA_LEDGER_ORIGIN_TOKEN` for Clear Signing; `post_trade` reports `clearSigning.isBlindSign` when the owner Ledger signed.
...
The hosted MCP canary requires `AQUA_PUBLIC_ORIGIN`. It calls origin `/mcp` `tools/list` and fingerprints the eight canonical tools. Aqua x402 remains exact Permit2 funding in the trade's sell token on the deployment chain.
