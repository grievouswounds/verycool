...
const signEip1559=async(parameters:{purpose:"keeper"|"facilitator";chainId:string;nonce:string;maxPriorityFeePerGas:string;maxFeePerGas:string;gas:string;to:Address;value:string;data:string})=>{const config=await manifest();const chainId=BigInt(parameters.chainId);const gas=BigInt(parameters.gas);const maxFeePerGas=BigInt(parameters.maxFeePerGas);const keeperAllowed=config.keeper.allowedTargets.includes(parameters.to)&&config.keeper.allowedSelectors.includes(parameters.data.slice(0,10));const facilitatorAllowed=parameters.to===config.contracts.x402ExactPermit2Proxy.address;const allowed=parameters.purpose==="keeper"?keeperAllowed:facilitatorAllowed;if(chainId!==BigInt(config.chain.id)||!allowed)throw new Error("Signer chain, target, or selector is outside policy");if(BigInt(parameters.value)!==0n||gas>8_000_000n||maxFeePerGas>100_000_000_000n)throw new Error("Signer value, gas, or fee exceeds policy");const signer=new CubaneTransactionSigner(parameters.purpose==="keeper"?keys.keeper:keys.facilitator);return {rawTransaction:signer.sign({chainId,nonce:BigInt(parameters.nonce),maxPriorityFeePerGas:BigInt(parameters.maxPriorityFeePerGas),maxFeePerGas,gas,to:parameters.to,value:0n,data:hexSchema.parse(parameters.data)})};};
