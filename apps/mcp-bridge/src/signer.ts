...
import { addressSchema, hexSchema, MAX_SUBPROCESS_BYTES, parseStrictJson, readBoundedText } from "@aqua/core";
...
const run = async (arguments_: readonly string[], input: string | null, inherit = false): Promise<string> => {
  ...
  const [stdout, stderr, exitCode] = await Promise.all([readBoundedText(new Response(child.stdout), MAX_SUBPROCESS_BYTES), inherit ? Promise.resolve("") : readBoundedText(new Response(child.stderr), MAX_SUBPROCESS_BYTES), child.exited]);
...
if (mode === "provision") {
  ...
  const request = z.object({ typedData: typedDataSchema }).strict().parse(parseStrictJson(await readBoundedText(new Response(Bun.stdin), MAX_SUBPROCESS_BYTES)));
  ...
  }).strict() }).strict().parse(parseStrictJson(await readBoundedText(new Response(Bun.stdin), MAX_SUBPROCESS_BYTES)));
