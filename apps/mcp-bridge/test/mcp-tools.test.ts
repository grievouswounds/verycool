...
const names = ["request_trade", "post_trade", "get_trades", "cancel_trade", "subscribe_to_user", "unsubscribe_from_user", "wipe_subscribed_trades", "get_balances"];
...
describe("hosted Aqua MCP catalog", () => {
  ...
  test("fails closed when the origin omits a required operation", async () => {
    ...
    })).rejects.toThrow(/missing required operation get_balances/);
  });
