...
import {
  ...
  encodeX402ExactSettle, encodeX402ExactSettleWithPermit, createPooledRpcClient, recoverTypedDataAddress, selector,
} from "@aqua/evm";
import type { Eip712TypedData } from "@aqua/evm";
import { createSecretBroker, type SecretBroker } from "@aqua/security";
import { z } from "zod";
...
export const createFacilitatorRoutes = async (
  manifest: RuntimeManifest,
  broker: SecretBroker,
) => {
  const identity = await broker.identity();
  const rpc = createPooledRpcClient(manifest.chain, localProfileDefaults.rpcTimeoutMs);
  ...
  const send = async (toValue: string, dataValue: string): Promise<WireHex> => {
    ...
    const [nonce, gas, gasPrice, priority] = await Promise.all([
      session.transactionCount(identity.facilitator), session.estimateGas({ from: identity.facilitator, to, data }),
      session.gasPrice(), session.maxPriorityFeePerGas(),
    ]);
    ...
    return wireHexSchema.parse(await session.sendRawTransaction(raw));
  };
...
const parsedRequest = async (request: Request): Promise<{ readonly paymentPayload: PaymentPayload; readonly paymentRequirements: PaymentRequirements }> => {
  ...
  const routes = {
    ...
    "/verify": { POST: async (request: Request) => { const body = await parsedRequest(request); return Response.json(await facilitator.verify(body.paymentPayload, body.paymentRequirements)); } },
    "/settle": { POST: async (request: Request) => { const body = await parsedRequest(request); return Response.json(await facilitator.settle(body.paymentPayload, body.paymentRequirements)); } },
...
if (import.meta.main) {
  ...
  const server = Bun.serve({
    hostname: Bun.env["AQUA_BIND_HOST"]?.trim() ?? host.hostname,
    port: Number(host.port),
    routes: await createFacilitatorRoutes(manifest, broker),
  });
