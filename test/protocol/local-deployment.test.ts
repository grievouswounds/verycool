...
describe("local deployment policy", () => {
  test("pins canonical Permit2 and x402 addresses and exact keeper selectors", async () => {
    ...
    expect(keccakHex(new TextEncoder().encode("execute((address,uint8,bytes,address[],uint256[],uint256),bytes)")).slice(0, 10)).toBe("0x3acc266e");
  ...
  test("requires BoundedMatcher and permits both fixture pair directions", () => {
    ...
    const input = {
      ...
      keeper: { allowedTargets: [contract("5").address, contract("6").address, contract("7").address], allowedSelectors: ["0xb1b0923a", "0x5f330b0f", "0x3acc266e", "0xc8d18a45"] },
